...
const productRoutes = async (server, options) => {
    ...
    server.post('/', async (request, reply) => {
        ...
        const { rows } = await server.pg.query('INSERT INTO products (name, description) VALUES ($1, $2) RETURNING *', [name, description]);
        return rows[0];
    });
    server.put('/:id', async (request, reply) => {
        const { id } = request.params;
        const { name, description } = request.body;
        const { rows } = await server.pg.query('UPDATE products SET name = $1, description = $2 WHERE id = $3 RETURNING *', [name, description, id]);
        if (rows.length > 0) {
            return rows[0];
        }
        else {
